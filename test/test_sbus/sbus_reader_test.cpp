...
void test_Read_Sbus_success(void)
{
    ...
    bfs::mock_sbus_ch[4] = 1500; // arm (aktywny, > SBUS_CENTER)
    bfs::mock_sbus_ch[5] = 1500; // mode (aktywny, > SBUS_CENTER)

    float status = reader.Read_Sbus();
    TEST_ASSERT_EQUAL_FLOAT(0.0, status);

    // Obliczone expected: map(1000, 172, 1811, 48, 2047) ≈ 1058
    TEST_ASSERT_EQUAL_FLOAT(1058.0, reader.get_throttle());
    TEST_ASSERT_EQUAL_FLOAT(1058.0, reader.get_yaw());
    TEST_ASSERT_EQUAL_FLOAT(1058.0, reader.get_pitch());
    TEST_ASSERT_EQUAL_FLOAT(1058.0, reader.get_roll());
    TEST_ASSERT_EQUAL_FLOAT(1.0, reader.get_arm_status()); // 1500 > 992
    TEST_ASSERT_EQUAL_FLOAT(1.0, reader.get_mode());       // 1500 > 992
}
